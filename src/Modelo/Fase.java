...
public class Fase {
    ...
    public void atualizar(double velocidadeScroll) {
        scrollY = (scrollY + velocidadeScroll) % 640;
        distanciaTotalRolada += velocidadeScroll;

        for(Projetil p : projeteis) { p.mover(); }
        projeteis.removeIf(p -> p.estaForaDaTela(640, 640));
    ...
    private void atualizarArvores(double velocidadeScroll) {
        ...
        if (imagemFundo2 != null && distanciaTotalRolada >= proximoSpawnY) {
            int tamanhoBase = (int) Math.round(640 * 0.8);
            boolean vaiBaterNaDireita = direcaoDoGrupo == 1 && (posicoesXDasDiagonais[NUMERO_DE_DIAGONAIS - 1] + tamanhoBase) > 640;
            ...
            for (int i = 0; i < NUMERO_DE_DIAGONAIS; i++) {
                ...
                arvores.add(new ArvoreParallax(novoX + randomOffsetX, yInicial, tamanhoBase, velocidadeScroll, imagemFundo2));
        ...
        arvores.removeIf(arvore -> arvore.estaForaDaTela(640));
    }
    
    
    // Getters para que a View (Cenario2) e o Controller (Engine) possam ler os dados
    ...
    private void preencherCenarioInicial() {
        ...
        while (proximoSpawnY < alturaTela) {
            // Lógica de spawn copiada de 'atualizarArvores'
            boolean vaiBaterNaDireita = direcaoDoGrupo == 1 && (posicoesXDasDiagonais[NUMERO_DE_DIAGONAIS - 1] + tamanhoBase) > alturaTela;
            boolean vaiBaterNaEsquerda = direcaoDoGrupo == -1 && posicoesXDasDiagonais[0] < 0;
            if (vaiBaterNaDireita || vaiBaterNaEsquerda) {
                direcaoDoGrupo *= -1;
            }
